...
class Slider:
    ...
    def __init__(self):
        """
        初始化：加载 ONNX 模型文件
        优先尝试 INT8 静态量化模型，如果加载失败则自动降级到 FP32
        """
        ...
        if os.path.exists(int8_path):
            try:
                self.session = ort.InferenceSession(int8_path, sess_options=so, providers=providers)
                print(f"✅ 加载静态量化模型: {int8_path}")
            except Exception as e:
                print(f"⚠️ INT8 模型加载失败: {e}")
                self.session = ort.InferenceSession(fp32_path, sess_options=so, providers=providers)
